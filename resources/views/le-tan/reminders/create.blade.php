        ...
        <form action="{{ route('le-tan.reminders.store') }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="reminder_date" name="reminder_date" value="{{ old('reminder_date') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                    @error('reminder_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="message" name="message" rows="5" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>{{ old('message', "Kính gửi [tên khách hàng],\n\nChúng tôi xin nhắc nhở bạn về lịch hẹn sắp tới tại Beauty Spa:\n\nDịch vụ: [tên dịch vụ]\nNgày: [ngày]\nGiờ: [giờ]\n\nVui lòng đến đúng giờ để được phục vụ tốt nhất.\n\nXin cảm ơn,\nBeauty Spa") }}</textarea>
                    @error('message')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
