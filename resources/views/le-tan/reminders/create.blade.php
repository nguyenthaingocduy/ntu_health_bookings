...
@section('title', 'Tạo nhắc lịch hẹn mới')

@section('header', 'Tạo nhắc lịch hẹn mới')

@section('content')
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Tạo nhắc lịch hẹn mới</h1>
            <p class="text-sm text-gray-500 mt-1">Tạo nhắc nhở lịch hẹn mới cho khách hàng</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('le-tan.reminders.store') }}" method="POST" class="p-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="reminder_type" name="reminder_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        ...
                        <option value="both" {{ old('reminder_type') == 'both' ? 'selected' : '' }}>Email & SMS</option>
                    </select>
                    @error('reminder_type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" id="reminder_date" name="reminder_date" value="{{ old('reminder_date') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                    @error('reminder_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="message" name="message" rows="5" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('message', "Kính gửi [tên khách hàng],\n\nChúng tôi xin nhắc nhở bạn về lịch hẹn sắp tới tại Beauty Spa:\n\nDịch vụ: [tên dịch vụ]\nNgày: [ngày]\nGiờ: [giờ]\n\nVui lòng đến đúng giờ để được phục vụ tốt nhất.\n\nXin cảm ơn,\nBeauty Spa") }}</textarea>
                    @error('message')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
