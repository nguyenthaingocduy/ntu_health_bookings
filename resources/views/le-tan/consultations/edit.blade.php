...
@section('title', 'Chỉnh sửa tư vấn dịch vụ')

@section('header', 'Chỉnh sửa tư vấn dịch vụ')

@section('content')
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Chỉnh sửa tư vấn dịch vụ</h1>
            <p class="text-sm text-gray-500 mt-1">Cập nhật thông tin tư vấn dịch vụ</p>
        </div>
        <div>
            <a href="{{ route('le-tan.consultations.show', $consultation->id) }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition-colors duration-150 flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('le-tan.consultations.update', $consultation->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="customer_id" name="customer_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        ...
                        <option value="{{ $customer->id }}" {{ (old('customer_id', $consultation->customer_id) == $customer->id) ? 'selected' : '' }}>
                            ...
                            @if($category->services->count() > 0 || $category->children->count() > 0)
                                <optgroup label="{{ $category->name }}">
                                    @foreach($category->services as $service)
                                        <option value="{{ $service->id }}" {{ (old('service_id', $consultation->service_id) == $service->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" id="recommended_date" name="recommended_date" value="{{ old('recommended_date', $consultation->recommended_date ? $consultation->recommended_date->format('Y-m-d') : '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="5" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('notes', $consultation->notes) }}</textarea>
                    @error('notes')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
