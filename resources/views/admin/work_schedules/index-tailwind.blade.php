                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="px-6 py-3 bg-gray-50 text-center text-xs font-medium text-gray-500 uppercase tracking-wider {{ $date->isToday() ? 'bg-blue-50' : '' }}">
                                    <div>{{ substr($date->locale('vi')->dayName, 0, 3) }}</div>
                                    ...
                                    <div class="space-y-2">
                                        @foreach($timeSlots as $timeSlot)
                                            @php
                                                ...
                                                $slotId = "slot-{$technician->id}-{$date->format('Y-m-d')}-{$timeSlot->id}";
                                            @endphp
                                            
                                            <div class="flex items-center justify-center">
                                                <label for="{{ $slotId }}" class="flex items-center cursor-pointer">
                                                    <input type="checkbox" id="{{ $slotId }}" 
                                                           class="schedule-checkbox h-4 w-4 text-pink-600 focus:ring-pink-500 border-gray-300 rounded transition-colors duration-150"
                                                           data-user-id="{{ $technician->id }}"
                                                           data-date="{{ $date->format('Y-m-d') }}"
                                                           data-time-slot-id="{{ $timeSlot->id }}"
                                                           {{ $isScheduled ? 'checked' : '' }}>
                                                    
                                                    <span class="ml-2 text-xs text-gray-700">{{ $timeStart }} - {{ $timeEnd }}</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        scheduleCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    parentLabel.appendChild(timeSlotIdInput);
                    
                    // Hiệu ứng khi chọn
                    this.classList.add('ring-2', 'ring-pink-300');
                    setTimeout(() => {
                        this.classList.remove('ring-2', 'ring-pink-300');
                    }, 300);
                } else {
                    // Nếu checkbox bị bỏ chọn, xóa input hidden
                    const inputs = parentLabel.querySelectorAll('input[type="hidden"]');
        ...
        selectAllDaysCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                const userId = this.dataset.userId;
                const userCheckboxes = document.querySelectorAll(`.schedule-checkbox[data-user-id="${userId}"]`);
        ...
        scheduleForm.addEventListener('submit', function(e) {
            ...
            if (scheduledInputs.length === 0) {
                ...
                alert('Vui lòng chọn ít nhất một khung giờ để phân công lịch làm việc.');
                return false;
            }
